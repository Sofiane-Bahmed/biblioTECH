...
describe("👤 User Profile Operations", () => {
  ...
  describe("GET /api/user/profile/me", () => {
    it("Should return the authenticated user's profile details", async () => {
      ...
      expect(res.body.data.profile.email).toBe("profile-test@test.com");
      expect(res.body.data.profile.fullName).toBe("Profile Test User");
  ...
  describe("PUT /api/user/profile/me/update-me", () => {
    it("Should update profile schema records matching body modifications", async () => {
      ...
      expect(res.body.data.user.email).toBe("updated-profile@test.com");
    });
  ...
  describe("GET /api/user/profile/me/borrows", () => {
    ...
    it("Should retrieve paginated logs with integrated book populations", async () => {
      ...
      expect(res.body.data.data[0].book.title).toBe("Profile Reference Book");
    });
